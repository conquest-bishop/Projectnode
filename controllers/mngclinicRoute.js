...
// clinic form route 
router.get('/mngclinicform', (req, res) => {
    res.render('mngclinic.pug')
});
...
router.post('/regclinic', async (req, res) => {
    try{
        ...
        res.redirect('/api/mngclinicform');
    }
    catch(error){
        res.status(404).render('mngclinic');
        console.log(error)}
});


// reading from table
router.get('/mngclinictable', async(req, res) =>{
    try{
        ...
        console.log(items)
        res.render('mngclinicTable.pug', {clients: items, count});
    }
    catch(error){
        console.log(error)
...
router.get('/mngclinic/edit/:id', async (req, res)=>{
    try{
        const vehicle = await Clinic.findOne({_id: req.params.id});
        res.render('mngeditclinic', {client:vehicle});
    }catch(error){
...
router.post('/mngclinic/edit', async (req, res)=>{
    try{
        await Clinic.findOneAndUpdate({_id: req.query.id}, req.body);
        res.redirect('/api/mngclinictable');
    }catch(error){
